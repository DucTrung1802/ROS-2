...
class SubtractClustering:
    ...
    def __init__(
        ...
        self.sf: cython.double = sf
        self.ar: cython.double = ar
        self.rr: cython.double = rr
    ...
    def __call__(self, *args, **kwargs) -> Tuple[np.ndarray, np.ndarray]:
        ...
        accum_multp: np.ndarray = 1.0 / self.radii
        ...
        for j in range(num_points):
            ...
            potential_values[j] = np.sum(
                [
                    np.exp(-4.0 * np.sum([dx[k, i] ** 2 for k in range(num_params)]))
                    for i in range(num_points)
                ]
            )
        num_clusters: cython.int = 0  # Количество кластеров
        max_potential: np.ndarray = (
            potential_values.max()
        )  # Точка с максимальным потенциалом
        ref_max_potential: np.ndarray = (
            max_potential  # Самый большой максимальный мотенциал
        )
        max_potential_index: cython.int = (
            potential_values.argmax()
        )  # Индекс максимального потенциала
        centers: List[np.ndarray] = []  # Центры кластеров
        find_more: cython.int = 1  # Флаг поиска центров класетров
        while find_more != 0 and max_potential != 0:
            find_more: cython.int = 0
            max_point: np.ndarray = x[:, max_potential_index]
            max_potential_ratio: np.ndarray = max_potential / ref_max_potential
            if (
                max_potential_ratio > self.ar
            ):  # Новое значение пика является значительным
                find_more: cython.int = 1
            elif (
                max_potential_ratio > self.rr
            ):  # Принято точку тогда, когда далеко от кластеров
                min_dest_sq = -1.0
                for center in centers:
                    dx_sq: np.ndarray = np.sum(
                        [
                            np.power((max_point[i] - center[i]) * accum_multp[i], 2)
                            for i in range(num_params)
                        ]
                    )
                    if min_dest_sq < 0 or dx_sq < min_dest_sq:
                        min_dest_sq = dx_sq
                    if max_potential_ratio + min_dest_sq ** 0.5 >= 1:
                        find_more = 1
                    else:
                        find_more = 2
            if find_more == 1:
                ...
                num_clusters += 1
                dx: np.ndarray = (max_point[:, np.newaxis] - x) * sqsh_multp[
                    :, np.newaxis
                ]
                deduct: np.ndarray = np.array(
                    [
                        ...
                        * np.exp(
                            -4
                            * np.sum([np.power(dx[i, j], 2) for i in range(num_params)])
                        )
                        for j in range(num_points)
                    ]
                )
                ...
                max_potential_index: cython.int = potential_values.argmax()
            elif find_more == 2:
                potential_values[max_potential_index] = 0.0
                max_potential: np.ndarray = potential_values.max()
                max_potential_index: cython.int = potential_values.argmax()
        # Денормализация данных с использованием min_x and max_x
        centers: np.ndarray = (centers * (max_x - min_x) + min_x).transpose()
        sigmas: np.ndarray = (self.radii * (max_x - min_x)) / 8 ** 0.5
        return centers, sigmas
